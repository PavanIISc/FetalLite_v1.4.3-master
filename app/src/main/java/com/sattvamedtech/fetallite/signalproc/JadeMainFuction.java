...
//			System.out.println("Jade-> sweep " + aSweep);
			...
			for (int p = 0; p < Constants.NO_OF_CHANNELS - 1; p++) {
				for (int q = p + 1; q < Constants.NO_OF_CHANNELS; q++) {
					...
					if (Math.abs(aTheta) > aSmallAngleTh) {
						...
						aCumulantMatrixTemp = mMatrixFunctions.multiply(mMatrixFunctions.transpose(aGivensMatrix), aCumulantMatrixExtract);
		...
		double[][] aWhiteningMatrix = mMatrixFunctions.multiply(mMatrixFunctions.transpose(aGivensRotationM), aSpheringMatrix);
