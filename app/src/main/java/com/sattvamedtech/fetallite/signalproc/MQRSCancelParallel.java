...
public class MQRSCancelParallel {
	...
	static Object[] mSVD = null;
	
	
	
	private double[][] svdParallized(int iNoQrs, int[] iQrsIndexArrI, int[] iQrsIndexArrF, double[][] iRowWeighted,
			...
			for (int it = 0; it < aNo_of_channels; it++) {
				...
				exec.submit(new Runnable() {
					...
					public void run() {
						...
						try {
							aApproxSignal = aISVD.impcitsvd(aSvdExtract);
	...
	public double[][] cancel(double[][] iInput, int[] iQRSm) throws Exception {
		...
		aApproxmSignal = svdParallized(aNoQrs, aQrsIndexArrI, aQrsIndexArrF, aRowWeighted, aRowextract, aInputSVD, aWeightWindowFunction, aNoSamplesQRS, aSvdExtract, aApproxmSignal);
