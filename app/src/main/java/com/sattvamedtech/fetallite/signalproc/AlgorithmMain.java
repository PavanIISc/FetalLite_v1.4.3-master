...
public class AlgorithmMain {
	...
	public Object[] algoStart(double[][] input, int iCurrentIteration) throws Exception {
		...
		double[][] aEcgIca1 = aMatrixFunctions.multiply(aEcgFilter, aMatrixFunctions.transpose(aWeightedMatrix1));
		 ...
		 */
		int aQRSF[];
		if (aQRSM.length >= 5 && aQRSM.length <= 30) {
...
//			double[][] aFetalSig = aCancel.cancel(aEcgFilter, aQRSM1);
			...
			double[][] aEcgIca2 = aMatrixFunctions.multiply(aFetalSig, aMatrixFunctions.transpose(aWeightedMatrix2));
			...
			if (aQRSF.length >= 13 && aQRSF.length <= 52) {
		...
		Logger.logInfo("ConversionHelper", "AlgorithmMain Time taken Iteration :" + Constants.CURRENT_ITERATION + " is : "+ (System.currentTimeMillis() - aStartTime));
...
//		double aDecimalRange = 1 - 584;
		...
		if (aQRS1 > 1500) {
			...
			aCurrentIndex = aQRS1 / Constants.DIFFERENCE_SAMPLES ;
		}
		...
		Logger.logInfo("ConversionHelper", " ConvertHR2MilliSec + "+ (System.currentTimeMillis() - aStartTime));
		return aHrPrint;
